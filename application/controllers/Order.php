...
class Order extends CI_Controller {
	...
	{ 
		if(isset($_GET['id_kota'])){
			$data['list'] = $this->m_general->gDataJW('kamar','fasilitas','id_kota',array('fasilitas.id_kota'=>$_GET['id_kota']))->result();
		}else{
	...
	public function make($id_kamar){
		$user = $this->session->userdata('id_user');
		$level = $this->session->userdata('level');
		if(!$user&&!$level){
			redirect('account/login');
		}
		$data['detail'] = $this->m_general->gDataJW('kamar','fasilitas','id_kota',array('id_kamar'=>$id_kamar))->row();
		$this->load->view('src/header',$data);
		$this->load->view('make',$data);
		$this->load->view('src/footer');
	}
	function processOrder($id_kamar){
		$user = $this->session->userdata('id_user');
		$level = $this->session->userdata('level');
		if(!$user&&!$level){
			redirect('account/login');
		...
		if(!empty($this->input->post())){
			...
			$insert['id_kamar'] = $id_kamar;
			$paket = $this->input->post('paket');
			$jumlah = $this->input->post('jumlah_paket');
			$detail = $this->m_general->gDataJW('kamar','fasilitas','id_kota',array('id_kamar'=>$id_kamar))->row();
			if($paket=='Permalam'){
				$hari = 1;
				$harga = $detail->harga;
	...
	public function my(){
		...
		$data['list'] = $this->m_general->gDataJW('pesanan','kamar','id_kamar',array('id_user'=>$id_user),'id_pesanan')->result();
		$this->load->view('src/header',$data);
